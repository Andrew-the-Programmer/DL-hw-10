...
def run_training(config: dict[str, Any], fast_train: bool = False) -> None:
    ...
    save_checkpoint_path = trainer_cfg.get("save_checkpoint_path")
    if save_checkpoint_path and Path(save_checkpoint_path).exists():
        print(f"Loading pretrained adapter from {save_checkpoint_path}")
        model.adapter.load_state_dict(torch.load(save_checkpoint_path, map_location="cpu"))
        print("Adapter loaded successfully.")
    else:
        print("No pretrained adapter loaded, starting from random initialization.")

...
def run_benchmark(config: dict[str, Any], toy: bool = False) -> dict[str, float]:
    ...
    for idx in tqdm(range(len(dataset)), desc="Evaluating"):
        ...
        print(f"Decoded (with specials): {tokenizer.decode(new_tokens, skip_special_tokens=False)}")
        print(f"Decoded (without specials): {tokenizer.decode(new_tokens, skip_special_tokens=True)}")
